...
const Profile = ({ role, currentUser }) => {
  ...
  const loadStudentClubs = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/studentClubs', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
        }
  ...
  const editCourse = (index, updatedCourse) => {
    setProfileData(prev => ({
      ...prev,
      coursesEnrolled: prev.coursesEnrolled.map((course, i) => 
  ...
  const editClub = (index, updatedClub) => {
    setProfileData(prev => ({
      ...prev,
      studentClubs: prev.studentClubs.map((club, i) => 
        i === index ? updatedClub : club
      )
    }));
    setEditingClub(null);
  };
    ...
    >
      <div className="profile-header">
        ...
        <p className="profile-subtitle">
          Customize your profile to connect better with your university community
        </p>
        {profileData.role && (
            ...
            <div className="form-grid">
              ...
              <div className="form-group form-group-full">
                ...
                <div className="courses-display">
                  {profileData.coursesEnrolled && Array.isArray(profileData.coursesEnrolled) && profileData.coursesEnrolled.length > 0 ? (
                    profileData.coursesEnrolled.map((course, index) => {
                      if (editingCourse === index) {
                        return (
                          <CourseForm
                            key={index}
                            course={course}
                            onChange={(updatedCourse) => editCourse(index, updatedCourse)}
                      ...
                      const courseCode = typeof course === 'string' ? course : 
                                       typeof course === 'object' ? (course.course || course.courseCode || course.code) : 
                                       'Unknown Course';
                      const courseName = typeof course === 'object' ? course.courseName : '';
                      const semester = typeof course === 'object' ? course.semester : '';
                      const year = typeof course === 'object' ? course.year : '';
                      const instructor = typeof course === 'object' ? course.instructor : '';
                      
                      return (
                        <div key={index} className="course-item">
                          <div className="course-content">
                            <strong>{String(courseCode)}</strong>
                            {courseName && <div>{String(courseName)}</div>}
                            {(semester || year) && (
                    ...
                    <div className="empty-state">
                      No courses enrolled yet. {selectedProgramId ? 'Click "Add Course" to add your first course.' : 'Please select a program first to add courses.'}
                    </div>
                  )}
                </div>
              </div>
              ...
              {profileData.role === 'Student' && (
                <div className="form-group form-group-full">
                  ...
                  <div className="clubs-display">
                    {profileData.studentClubs && Array.isArray(profileData.studentClubs) && profileData.studentClubs.length > 0 ? (
                      profileData.studentClubs.map((club, index) => {
                        if (editingClub === index) {
                          return (
                            <ClubForm
                              key={index}
                              club={club}
                              onChange={(updatedClub) => editClub(index, updatedClub)}
                        ...
                        const clubName = typeof club === 'string' ? club : 
                                       typeof club === 'object' ? (club.club || club.name || club.clubName) : 
                                       'Unknown Club';
                        const designation = typeof club === 'object' ? club.designation : '';
                        
                        return (
                          <div key={index} className="club-item">
                            <div className="club-content">
                              <strong>{String(clubName)}</strong>
                              {designation && (
                                <div style={{ fontSize: '12px', color: '#6c757d' }}>
                      ...
                      <div className="empty-state">
                        Not part of any student clubs yet. Click "Add Club" to join your first club.
                      </div>
                    )}
                  </div>
                </div>
            ...
            >
              ...
              {availableClubs.map(clubOption => (
                <option key={clubOption._id} value={clubOption.name || clubOption.clubName}>
                  {clubOption.name || clubOption.clubName}
                </option>
              ))}
            </select>
          </div>
